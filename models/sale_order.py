...
class sale_order_expiration_reminder(models.Model):
    ...
    def _cron_sale_order_consultancy_expiration_reminder(self):
        ...
        # Expires now
        fill_remind("new", [('consultancy_expiration_selection', 'in', ['in_progress']), '&', ('consultancy_expiration_date', '!=', False), ('consultancy_expiration_date', '<=', datetime.now().strftime('%Y-%m-%d'))], True)
        ...
        template_id = self.env["ir.model.data"].get_object_reference('sale_order_expiration_and_reminder', 'sale_order_expiration_reminder_cron_mail_template')[1]
